...
const getRentEstimate = async ({ address, city, state, zipCode, bedrooms, bathrooms, squareFootage, propertyType }) => {
  try {
    ...
    console.log('🏠 RentCast Rent API:', `${BASE_URL}/avm/rent/long-term?${params}`);
    ...
    if (!response.ok) {
      console.error('❌ RentCast Rent Error:', response.status);
      return null;
    }
...
const getPropertyDetails = async ({ address, city, state, zipCode }) => {
  try {
    ...
    console.log('🏠 RentCast Property API:', `${BASE_URL}/properties?${params}`);
    ...
    if (!response.ok) {
      console.error('❌ RentCast Property Error:', response.status);
      return null;
    }
...
export const getPropertyDataOnHover = async (property) => {
  try {
    ...
    console.log(`🔍 [HOVER] Fetching RentCast data for: ${address}`);
